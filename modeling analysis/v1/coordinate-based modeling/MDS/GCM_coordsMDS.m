function [Pr_resp_pat,Pr_corr_pat,Pr_corr] = GCM_coords(params,weights)
...
gamma = params.gamma;
load("coord_MDS.mat");
coord_train = coord_mds_test_old';
coord_test_old = coord_mds_test_old';
coord_test_newlow = coord_mds_test_newlow';
coord_test_newmed = coord_mds_test_newmed';
coord_test_newhigh = coord_mds_test_newhigh';
coord_proto = coord_mds_proto';
pat_test_newhigh_special = coord_mds_newhigh_special';
cat_train = repelem([1,2,3],[9,9,9]);

Pr_resp_proto = NaN([3,3]);
for ipat = 1:3
    sim_by_cat_proto = coord2sim(coord_train,coord_proto(:,ipat),cat_train,c,weights);
    Pr_resp_proto(ipat,:) = (sim_by_cat_proto.^gamma)/sum(sim_by_cat_proto.^gamma);
end

Pr_resp_old = NaN([27,3]);
for ipat = 1:27
    sim_by_cat_old = coord2sim(coord_train,coord_test_old(:,ipat),cat_train,c,weights);
    Pr_resp_old(ipat,:) = sim_by_cat_old.^gamma/sum(sim_by_cat_old.^gamma);
end

Pr_resp_newlow = NaN([9,3]);
for ipat = 1:9
    sim_by_cat_newlow = coord2sim(coord_train,coord_test_newlow(:,ipat),cat_train,c,weights);
    Pr_resp_newlow(ipat,:) = (sim_by_cat_newlow.^gamma)/sum(sim_by_cat_newlow.^gamma);
end

Pr_resp_newmed = NaN([18,3]);
for ipat = 1:18
    sim_by_cat_newmed = coord2sim(coord_train,coord_test_newmed(:,ipat),cat_train,c,weights);
    Pr_resp_newmed(ipat,:) = (sim_by_cat_newmed.^gamma)/sum(sim_by_cat_newmed.^gamma);
end

Pr_resp_newhigh = NaN([27,3]);
for ipat = 1:27
    sim_by_cat_newhigh = coord2sim(coord_train,coord_test_newhigh(:,ipat),cat_train,c,weights);
    Pr_resp_newhigh(ipat,:) = (sim_by_cat_newhigh.^gamma)/sum(sim_by_cat_newhigh.^gamma);
end

Pr_resp_newhigh_special = NaN([3,3]);
for ipat = 1:3
    sim_by_cat_newhigh_special = coord2sim(coord_train,pat_test_newhigh_special(:,ipat),cat_train,c,weights);
