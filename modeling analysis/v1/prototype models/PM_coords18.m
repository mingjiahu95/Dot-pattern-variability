function [Pr_resp_pat,Pr_corr_pat,Pr_corr] = PM_coords18(params,condition)
...
pat_test_newhigh_special = reshape(permute(coord_test_newhigh_hard,[2 1 3]),[18,3]);
cat_proto = [1,2,3];

Pr_resp_proto = NaN([3,3]);
for ipat = 1:3
    sim_by_cat_proto = coord2sim(pat_proto,pat_proto(:,ipat),cat_proto,c);
    Pr_resp_proto(ipat,:) = (sim_by_cat_proto.^gamma)/sum(sim_by_cat_proto.^gamma);
end

Pr_resp_old = NaN([27,3]);
for ipat = 1:27
    sim_by_cat_old = coord2sim(pat_proto,pat_test_old(:,ipat),cat_proto,c);
    Pr_resp_old(ipat,:) = sim_by_cat_old.^gamma/sum(sim_by_cat_old.^gamma);
end

Pr_resp_newlow = NaN([9,3]);
for ipat = 1:9
    sim_by_cat_newlow = coord2sim(pat_proto,pat_test_newlow(:,ipat),cat_proto,c);
    Pr_resp_newlow(ipat,:) = (sim_by_cat_newlow.^gamma)/sum(sim_by_cat_newlow.^gamma);
end

Pr_resp_newmed = NaN([18,3]);
for ipat = 1:18
    sim_by_cat_newmed = coord2sim(pat_proto,pat_test_newmed(:,ipat),cat_proto,c);
    Pr_resp_newmed(ipat,:) = (sim_by_cat_newmed.^gamma)/sum(sim_by_cat_newmed.^gamma);
end

Pr_resp_newhigh = NaN([27,3]);
for ipat = 1:27
    sim_by_cat_newhigh = coord2sim(pat_proto,pat_test_newhigh(:,ipat),cat_proto,c);
    Pr_resp_newhigh(ipat,:) = (sim_by_cat_newhigh.^gamma)/sum(sim_by_cat_newhigh.^gamma);
end

Pr_resp_newhigh_special = NaN([3,3]);
for ipat = 1:3
    sim_by_cat_newhigh_special = coord2sim(pat_proto,pat_test_newhigh_special(:,ipat),cat_proto,c);
