function [Pr_resp_pat,Pr_corr_pat,Pr_corr] = GCM(params,ndim)
...
for icond = 1:n_cond
    for itype = 1:n_type
        ...
        for iexem_cat = 1:n_cat
            testpat_cat = trainpat{icond,1};
            testpat(1,:) = testpat_cat(1,:); %an arbitrary old pattern from the target category
            if icond == 4 && itype == 1
                testpats_mix = testpat_cat([1,31,61],:);
            end
            sim = zeros(1,n_train); % store the similarity of the test pattern to each training pattern
            % for each category
            for iexem = 1:n_train
                train_item_cat = trainpat{icond,iexem_cat};
                train_item = train_item_cat(iexem,:);% coord of a training pattern as a vector
                if icond == 4 && itype == 1
                    test_items = testpats_mix;
                    sims(iexem,:) = exp(-c * sqrt(sum((train_item - test_items).^2,2)));
                else
                    test_item = testpat(itype,:); % coord of a test pattern as a vector
                    sim(iexem) = exp(-c * sqrt(sum((train_item - test_item).^2)));% similarity measure between the two
                end
            end
            % summed similarity for each category + response scaling
            if icond == 4 && itype == 1
                sim_sums = sum(sims,1); % summed similarity for each category
                sim_cats_old(iexem_cat,:) = sim_sums.^gamma;
            else
                sim_sum = sum(sim); % summed similarity for each category
                sim_cats(iexem_cat) = sim_sum^gamma;
            end
        end
