function [Pr_resp_pat,Pr_corr_pat,Pr_corr] = CM_activationMDS(params,weights)
%condition: 1 low 2 med 3 high 4 mix
%item type: 1 proto 2 old 3 newlow 4 newmed 5 newhigh 6 newhigh_hard
clearvars -except weights params condition
c = params.c;
gamma = params.gamma;
load("rotated_MDS_med.mat");
feature_train = mds_test_old;
feature_test_old = mds_test_old;
feature_test_newlow = mds_test_newlow;
feature_test_newmed = mds_test_newmed;
feature_test_newhigh = mds_test_newhigh;
feature_proto = mds_proto;
feature_test_newhigh_special = mds_newhigh_special;
cat_train_center = [1 2 3];
feature_train_center = [mean(feature_train(:,1:9),2),mean(feature_train(:,10:18),2),mean(feature_train(:,19:27),2)];


Pr_resp_old = NaN([27,3]);
for ipat = 1:27
    sim_by_cat_old = coord2sim(feature_train_center,feature_test_old(:,ipat),cat_train_center,c,weights);
    Pr_resp_old(ipat,:) = sim_by_cat_old.^gamma/sum(sim_by_cat_old.^gamma);
end

Pr_resp_newlow = NaN([9,3]);
for ipat = 1:9
    sim_by_cat_newlow = coord2sim(feature_train_center,feature_test_newlow(:,ipat),cat_train_center,c,weights);
    Pr_resp_newlow(ipat,:) = (sim_by_cat_newlow.^gamma)/sum(sim_by_cat_newlow.^gamma);
end

Pr_resp_newmed = NaN([18,3]);
for ipat = 1:18
    sim_by_cat_newmed = coord2sim(feature_train_center,feature_test_newmed(:,ipat),cat_train_center,c,weights);
    Pr_resp_newmed(ipat,:) = (sim_by_cat_newmed.^gamma)/sum(sim_by_cat_newmed.^gamma);
end

Pr_resp_newhigh = NaN([27,3]);
for ipat = 1:27
    sim_by_cat_newhigh = coord2sim(feature_train_center,feature_test_newhigh(:,ipat),cat_train_center,c,weights);
    Pr_resp_newhigh(ipat,:) = (sim_by_cat_newhigh.^gamma)/sum(sim_by_cat_newhigh.^gamma);
end

Pr_resp_newhigh_special = NaN([3,3]);
for ipat = 1:3
    sim_by_cat_newhigh_special = coord2sim(feature_train_center,feature_test_newhigh_special(:,ipat),cat_train_center,c,weights);
    Pr_resp_newhigh_special(ipat,:) = (sim_by_cat_newhigh_special.^gamma)/sum(sim_by_cat_newhigh_special.^gamma);
end

Pr_resp_proto = NaN([3,3]);
for ipat = 1:3
    sim_by_cat_proto = coord2sim(feature_train_center,feature_proto(:,ipat),cat_train_center,c,weights);
...
function [sim_by_cat,ncat,ntrain] = coord2sim(train_coords,test_coord,train_cats,sensitivity,weights)
...
for icat = 1:ncat
    ...
    dist_coords = sqrt(sum(weights'.*(diff_coords.^2),1));
