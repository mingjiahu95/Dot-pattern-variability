...
plot_bars <- function(data,title){
  ...
  p = ggplot(data, aes(x = condition, y = PropCorr, fill = measure)) +
    ...
    geom_text(aes(label = round(PropCorr,3)),size = 3,
              vjust = -.2, position = position_dodge(.7)) +
    xlab("Condition") +
    ylab("Proportion Correct") +
    ylim(0,1) +
    scale_fill_discrete(labels = c('standard','corrected')) +
    ggtitle(title,subtitle = sprintf("N(%s) = %i  N(%s) = %i N(%s) = %i  N(%s) = %i",cond_names[1],sample_size_info[1],cond_names[2],sample_size_info[2],cond_names[3],sample_size_info[3],cond_names[4],sample_size_info[4])) +
    theme_bw() + 
