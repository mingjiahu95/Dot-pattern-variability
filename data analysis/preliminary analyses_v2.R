...
# ---- Load the Data ----
setwd("../data/v2")
files = dir(pattern = ".txt") #change it back to ".txt" "sub(2[0-9][1-9])|([3-9][0-9]{2}).txt"
...
p = ggplot(data = learning_curve_df,
  ...
  geom_line(aes(linetype = condition),linewidth = .7) +
...
ggsave(paste('learning curve',".jpg"),plot = p ,path = "../figure/v2",width = 6, height = 6)
#-------------------------------------------------------------
...
ggsave(paste("test accuracy",".jpg"),plot = p ,path = "../figure/v2",width = 12, height = 6)
...
ggsave("training accuracy distribution.jpg",plot = p ,path = "../figure/v2",width = 6, height = 6)



#-------------------------------------------------------------------------------------------------------------
# histogram of test accuracy
hist_df = filter(data,phase == 2 & itemtype != 1) %>%
  group_by(condition,ID) %>%
  summarize(Pr_corr_sub = mean(correct == 1))
N_subj1 = length(unique(hist_df$ID[hist_df$condition == 1]))
N_subj2 = length(unique(hist_df$ID[hist_df$condition == 2]))
...
p <- ggplot(hist_df, aes(x=Pr_corr_sub)) + 
  geom_histogram(fill="white", color="black",binwidth = .05) + 
  ggtitle("novel test pattern accuracy by conditions") +
  xlab('Proportion Correct') +
  ylab('Number of subjects') +
  stat_bin(geom='text', color='black', aes(label=..count..),
           vjust = -.5,binwidth = .05) +
...
ggsave("test accuracy distribution.jpg",plot = p ,path = "../figure/v2",width = 6, height = 6)
